...
{
    private readonly ConcurrentDictionary<ICommandWithEntityKeys, CommandContext> _commandContext = new();
    ...
    public void StartFor<TRequest>(TRequest command, CommandContext context) where TRequest : ICommandWithEntityKeys
    {
    ...
    public CommandContext StartFor<TRequest>(TRequest command) where TRequest : ICommandWithEntityKeys
    {
    ...
    public void EndFor<TRequest>(TRequest command) where TRequest : ICommandWithEntityKeys
    {
        if (!_commandContext.TryRemove(command, out _))
            throw new InvalidOperationException("No context found for this command");
    }

    public async Task<ICommandContext> GetForAsync<TRequest>(TRequest command, CancellationToken cancellationToken = default) where TRequest : ICommandWithEntityKeys
    {
        try
        {
