...
/// Сохраняет контекст для конкретной команды (IRequest), чтобы к нему можно было обращаться во время выполнения конвейера (pipeline).
/// </summary>
internal class CommandContextManager : ICommandContextSource
{
    ...
    public void StartFor<TRequest>(TRequest command, CommandContext context) where TRequest : ICommandWithEntityKeys, IRequest
    {
    ...
    public CommandContext StartFor<TRequest>(TRequest command) where TRequest : ICommandWithEntityKeys, IRequest
    {
    ...
    public void EndFor<TRequest>(TRequest command) where TRequest : ICommandWithEntityKeys, IRequest
    {
        if (!_commandContext.TryRemove(command, out _))
            throw new InvalidOperationException("No context found for this command");
    }

    public async Task<ICommandContext> GetForAsync<TRequest>(TRequest command, CancellationToken cancellationToken = default) where TRequest : ICommandWithEntityKeys, IRequest
    {
